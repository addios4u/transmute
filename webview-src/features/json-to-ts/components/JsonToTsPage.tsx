...
export function JsonToTsPage() {
  ...
  const { input, setInput, options, setOptions, result, convert } = useJsonToTs();
      ...
      <div className="grid min-h-0 flex-1 grid-cols-2 gap-4">
        ...
        <div className="flex flex-col overflow-hidden rounded-xl border border-border/50 bg-bg-secondary/50 shadow-sm">
          <div className="flex items-center justify-between border-b border-border/50 bg-bg-tertiary/30 px-4 py-2.5">
            <span className="text-sm font-medium text-text-secondary">TypeScript</span>
            {result?.typescript && <CopyButton text={result.typescript} onCopy={handleCopy} />}
          </div>
          <div className="min-h-0 flex-1">
            <MonacoEditorWrapper
              value={result?.typescript ?? ""}
              language="typescript"
              readOnly
            />
          </div>
